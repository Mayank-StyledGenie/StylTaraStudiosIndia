...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error in photographer registration:', error);
    return NextResponse.json({ 
      success: false, 
      message: 'Failed to register photographer' 
    }, { status: 500 });
  }
}

async function sendConfirmationEmails(photographerData: PhotographerData) {
  ...
  const photographerMailOptions: MailOptions = {
    from: process.env.MAIL_USER as string,
    to: photographerData.email,
    subject: 'StylTara Studios: Photographer Registration Confirmation',
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
      ...
      <body>
        <div class="email-container">
          ...
          <div class="email-body">
            ...
            <p class="message">We've received your application for the Photographer position and would like to thank you for your interest in career opportunities with StylTara Studios Pvt Ltd.</p>
  ...
  const adminMailOptions: MailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0;">
        <h2 style="color: #401735; border-bottom: 2px solid #401735; padding-bottom: 10px;">New Photographer Registration</h2>
        <p>A new photographer has registered on your platform.</p>
        
        <h3 style="color: #4a5568;">Photographer Details:</h3>
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Name:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.fullName ?? photographerData.firstName ?? ''} ${photographerData.lastName ?? ''}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Email:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.email ?? 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Phone:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.phone ?? 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Experience Level:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.experienceLevel ?? 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Equipment:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.equipment ?? 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;"><strong>Registration Date:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${photographerData.createdAt.toLocaleString()}</td>
          </tr>
        </table>
  ...
  if (photographerData.portfolioFile?.data) {
    adminMailOptions.attachments?.push({
      filename: photographerData.portfolioFile.name,
      content: photographerData.portfolioFile.data,
      contentType: photographerData.portfolioFile.type
    });
  }

  try {
    await transporter.sendMail(photographerMailOptions);
    await transporter.sendMail(adminMailOptions);
    console.log('Confirmation emails for photographer registration sent successfully');
  } catch (error) {
    console.error('Error sending photographer confirmation emails:', error);
  }
}